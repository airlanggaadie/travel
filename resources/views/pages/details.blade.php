                    ...
                    <div class="col-lg-4">
                        ...
                        <table class="trip-informations">
                            <tr>
                                ...
                                <td width="50%" class="text-right" style="color:black;">{{ \Carbon\Carbon::create($item->date_of_departure)->format('F n, Y') }}</td>
                            </tr>
                            <tr>
                                <th width="50%" style="color:black;">Duration</th>
                                <td width="50%" class="text-right" style="color:black;">{{ $item->duration }}</td>
                            </tr>
                            <tr>
                                <th width="50%" style="color:black;">Type</th>
                                <td width="50%" class="text-right" style="color:black;">{{ $item->type }}</td>
                            </tr>
                            <tr>
                                <th width="50%" style="color:black;">Price</th>
                                <td width="50%" class="text-right" style="color:black;">${{ $item->price }},00 / person</td>
                            </tr>
                        </table>
                        </div>
                        <div class="join-container">
                            @auth
                                <form action="{{ route('checkout-process',$item->id) }}" method="post">
                                    @csrf
                                    <button type="submit" class="btn btn-block btn-join-now mt-3 py-2" style="color:black">Join</button>
                                </form>
                            @endauth
                            @guest
                                <a href="{{ route('login') }}" style="color:black;" class="btn btn-block btn-join-now mt-3 py-2">Login atau Register untuk masuk</a>                                
                            @endguest
                        </div>
                    </div>
                </div>
